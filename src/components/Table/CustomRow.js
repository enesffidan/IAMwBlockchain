...
export default class CustomRow extends React.Component {
  ...
  renderActions() {
    const size = this.getElementSize()
    const actions = this.props.actions.filter(a => !a.isFreeAction || a.position === "auto")
    return (
      <TableCell
        size={size}
        padding="checkbox"
  ...
  renderSelectionColumn() {
    ...
    const styles =
      size === 'medium'
        ...
        : {
            padding: '4px',
            marginLeft: 50 + this.props.level * 9,
          }
      ...
      >
        <Checkbox
          ...
          onChange={event =>  {
            ...
            this.props.data.tableData.selection = !this.props.data.tableData.selection;
          }}
          style={styles}
          {...checkboxProps}
        />
  ...
  shouldComponentUpdate(nextProps, nextState) {
    const {selectableEnabled, columns, row, data} = this.props;
    //console.log(this.props, nextProps);
    let shouldUpdate = false;
    if (nextProps.selectableEnabled !== selectableEnabled ||
        nextProps.isSelected !== data.tableData.checked ||
        nextProps.row !== row
    ) {
        shouldUpdate = true;
    }
    ...
    if (JSON.stringify(nextState?.details) !== JSON.stringify(this.state?.details)) {
      shouldUpdate = true;
    }
    return shouldUpdate;
  } 
